...
interface FlickrAPI {
    ...
    fun getRecent(@Query("api_key") apiKey: String = BuildConfig.API_KEY): LiveData<ApiResponse<List<PhotoDTO>>>

    @GET("/services/rest/?method=flickr.photos.getPopular&format=json&nojsoncallback=1")
    fun getPopular(@Query("api_key") apiKey: String = BuildConfig.API_KEY): LiveData<ApiResponse<List<PhotoDTO>>>
